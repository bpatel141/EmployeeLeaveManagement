...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form id="employeeForm" method="POST" action="{{ route('admin.employees.store') }}" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input 
                        ...
                        oninput="updateGeneratedPassword()"
                    />
                    @error('name')
                ...
                <div class="mt-4 p-4 bg-red-50 border border-red-200 rounded-md">
                    <label class="block text-sm font-medium text-red-800 mb-2">⚠️ Important Password Information</label>
                    <div class="text-sm text-red-700">
                        <p class="font-semibold text-red-800 mb-2">📧 No Email Functionality Available</p>
                        <p class="text-red-700 mb-3">Currently we do not have email functionality, so please share the generated password <strong class="text-red-800">offline</strong> with the employee for login.(Ex :- John@123)</p>
                    </div>
                </div>
...
function updateGeneratedPassword() {
    ...
    if (name) {
        // Generate password in format: Name@123
        const generatedPassword = name.charAt(0).toUpperCase() + name.slice(1).toLowerCase() + '@123';
        generatedPasswordSpan.textContent = generatedPassword;
    } else {
        generatedPasswordSpan.textContent = 'Type name above to see password';
    }
}

// Validation functions removed - using only Laravel validation
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Real-time email validation removed - using only Laravel validation
});
</script>
@endsection
